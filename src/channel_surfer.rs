    ...
    fn format_time_pair(current_secs: f64, total_secs: f64) -> (String, String) {
        ...
        if total_hours > 0 {
            ...
            (
                format!(
                    "{:02}:{:02}:{:02}",
                    current_hours, current_mins, current_secs_rem
                ),
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        ctx.input(|i| {
            ...
            if i.key_pressed(egui::Key::G) {
                ...
                println!(
                    "Glitch mode: {}",
                    if self.glitch_mode { "ON" } else { "OFF" }
                );
            }
            ...
            if scroll_delta.y > 0.0 {
                ...
                self.active_effect = match self.active_effect {
                    ...
                    VideoEffect::Contrast { factor } if factor > 1.5 => VideoEffect::Contrast {
                        factor: factor - 0.5,
                    },
                    VideoEffect::Contrast { .. } => VideoEffect::Invert,
                    VideoEffect::Compression { quality } if quality < 40 => {
                        VideoEffect::Compression {
                            quality: quality * 2,
                        }
                    }
                    ...
                    VideoEffect::MotionGlitch { .. } => VideoEffect::Glitch {
                        intensity: 10,
                        seed: 0,
                    },
                    VideoEffect::Datamosh { displacement } if displacement < 0 => {
                        VideoEffect::Datamosh {
                        ...
                        if duration_ms > 0.0 {
                            ...
                            if response.dragged() || response.clicked() {
                                if let Some(pointer_pos) = response.interact_pointer_pos() {
                                    let relative_x =
                                        (pointer_pos.x - rect.min.x).max(0.0).min(rect.width());
                            ...
                            ui.horizontal(|ui| {
                                ...
                                let (current_formatted, total_formatted) =
                                    Self::format_time_pair(current_secs, total_secs);
                                ui.label(
                                    egui::RichText::new(format!(
                                        "{} / {}",
                                        current_formatted, total_formatted
                                    ))
                                    .size(13.0)
                                    .color(egui::Color32::LIGHT_GRAY),
                                );
...
fn main() -> eframe::Result<()> {
    ...
    let video_exts = [
        "mp4", "mov", "avi", "mkv", "webm", "flv", "wmv", "m4v", "mpg", "mpeg", "3gp", "3g2", "ogv",
    ];
    // Extensions to explicitly exclude (images, audio, etc.)
    let exclude_exts = [
        "png", "jpg", "jpeg", "gif", "bmp", "tiff", "webp", "svg", "heic", "raw", "mp3", "wav",
        "flac", "aac", "ogg", "wma", "m4a", "txt", "json", "xml", "pdf", "doc", "zip", "tar", "gz",
        "dmg", "app", "exe",
    ];
